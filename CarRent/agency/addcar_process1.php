...
if ($result->num_rows > 0) {
    ...
    if (!empty($_FILES["car_image"]["name"])) {
        ...
        // Define the relative path for storing and displaying the image
        $relative_path = "/Car-Rental-System-main/CarRent/uploads/" . $imagename;
        $absolute_path = $_SERVER['DOCUMENT_ROOT'] . $relative_path; // Full server path for move_uploaded_file

        // Move the uploaded file to the target directory
        if (move_uploaded_file($temp_name, $absolute_path)) {
            // Save only the relative path in the database
            $query = "INSERT INTO cars (car_model, car_number, seating_cap, rent_per_day, car_img, car_available) 
                      VALUES ('$car_model', '$car_number', '$seating_cap', '$rent_per_day', '$relative_path', '$car_availability')";
